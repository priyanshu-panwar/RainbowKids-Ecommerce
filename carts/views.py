...
def SendtoPaytm(request):
    ...
    if request.POST.get('submit')=='button1':
        ...
        param_dict = {
            ...
            'WEBSITE': 'WEBSTAGING',
            'CHANNEL_ID': 'WEB',
            'CALLBACK_URL': 'https://therainbows.herokuapp.com/carts/handlerequest/', #'https://therainbows.herokuapp.com/carts/handlerequest/',
        }
        pincode = request.POST.get('pincode')
        details = request.session['items']
        obj = Order.objects.create(username=request.user, order_id=str(data['order_id']), address=data['address'],pincode=str(pincode),total=str(float(total)),
    ...
    if request.POST.get('submit') == 'button2':
        ...
        details = request.session['items']
        obj = Order.objects.create(username=request.user, order_id=str(data['order_id']), address=data['address'],pincode=str(pincode),
